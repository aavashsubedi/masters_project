...
def path_to_graph_two(graph, weight_image):
    ...
    #issue is the weight_image we are passing is the label isntead of the actual weight.
    ...
    graph.target = target
    shape = graph.centroid_values.shape
    x = torch.normal(0, 1, shape)
    graph.x = x
    
    #randomly generate x values from gaussian distribution
    #can replace this with glorot or xavier initialisation
    ...
    nx_graph.x = graph.centroid_values
  ...
  #  path = nx.shortest_path(nx_graph, source=0, target=target)
    ...
    cost_path = torch.sum(graph.centroid_in_path * graph.centroid_values).item()
    graph.cost_path = torch.tensor(cost_path)
